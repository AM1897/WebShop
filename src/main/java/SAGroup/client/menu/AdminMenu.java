...
public class AdminMenu {
    ...
    public static void showMenu(String token) {
        while (true) {
            ...
            System.out.println("6. Se en specifik användares kundvagnshistorik");
            ...
            switch (choice) {
                ...
                case 3:
                    ...
                    break;
                case 4:
                    ArticleMenu.showMenu(token);
                    break;
                case 5:
                    showShoppingCartHistory(token);
                    break;
                case 6:
                    showSpecificUserShoppingCartHistory(token);
                    break;
                case 7:
    ...
    private static void showSpecificUserShoppingCartHistory(String token) {
        System.out.print("Ange användarnamn: ");
        String userName = scanner.nextLine();
        HistoryClient historyClient = new HistoryClient("http://localhost:8080", token);

        try {
            String history = historyClient.getCheckoutHistoryOfUser(userName);
            System.out.println("Användarens " + userName + " kundvagnshistorik:\n" + history);
        } catch (IOException | InterruptedException e) {
            System.out.println("Kunde inte hämta historiken för användare " + userName + ": " + e.getMessage());
        }
    }
